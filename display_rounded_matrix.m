...
#Significative ciphers transfer to a cell:
  ...
  for i = 1:columns(significative) #transfering the precision into a cell of numbers.
    precision_format{i,1} = strcat("%.", num2str(significative(1,i)-1), "e"); %To compensate the absurd that significative ciphers are NOT decimals...
  endfor;
...
elseif (columns(significative) == columns(matrix)) #Every column has its own number of cifers.
  for i = 1:row #For every row in the matrix:
	...
	disp("significative:");
	disp(significative);
	error("display_rounded_matrix: Significative values different from row number in the input matrix.");
endif;
